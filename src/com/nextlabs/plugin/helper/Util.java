package com.nextlabs.plugin.helper;
...
public class Util {
	...
	public static String findInstallFolder() {
		...
		String path = Util.class.getProtectionDomain().getCodeSource()
				.getLocation().getPath();
		...
		try {
			...
			LOG.error(String.format("Exception while decoding the path: %s",
					path), e);
		}

		 int endIndex = path.indexOf("jservice/jar");
		//int endIndex = path.indexOf("build/classes");
		if (isWindows()) {
